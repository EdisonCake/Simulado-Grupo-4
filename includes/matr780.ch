#ifdef SPANISH
	#define STR0001 "Estadisticas de Ventas (Cliente vs. Producto)"
	#define STR0002 "Este programa emite el detalle de las compras efectuadas por el Cliente,"
	#define STR0003 "totalizando por producto y eligiendo la moneda fuerte para los Valores."
	#define STR0004 "A Rayas"
	#define STR0005 "Administracion"
	#define STR0006 "ESTADISTICAS DE VENTAS (Cliente vs. Producto)"
	#define STR0007 "CLIENTE   RAZON SOCIAL"
	#define STR0008 "PRODUTO         DESCRIPCION                  FACTURA         EMISION    UN    CANTIDAD      PRECIO UNITARIO            TOTAL  VENDEDOR"
	#define STR0009 "CLIENTE  RAZON SOCIAL"
	#define STR0010 "Valores en "
	#define STR0011 "Seleccionando registros..."
	#define STR0012 "ANULADO POR EL OPERADOR"
	#define STR0013 "Obs.: "
	#define STR0014 "TOTAL DEL PRODUCTO - "
	#define STR0015 "TOTAL DEL CLIENTE - "
	#define STR0016 "T O T A L   G E N E R A L                    ---->"
	#define STR0017 "DEV"
	#define STR0018 "Estadisticas de Ventas (Cliente vs. Producto)"
	#define STR0019 "Este programa emitira la lista de compras efectuada por el Cliente,"
	#define STR0020 "totalizando por producto y eligiendo la moneda fuerte para los Valores."
	#define STR0021 "Valores en "
	#define STR0022 "TOTAL DEL PRODUCTO - "
	#define STR0023 "TOTAL DEL CLIENTE - "
	#define STR0024 "Vendedor"
	#define STR0025 "DEV"
	#define STR0026 "Seleccionando Registros..."	
	#define STR0027 "Clientes"	
	#define STR0028 "Items de Facturas de Salida"	
	#define STR0029 "Devoluc."	
#else
	#ifdef ENGLISH
		#define STR0001 "Sales Statistics      (Customer x Product)"
		#define STR0002 "This program will print a list of purchases carried out by the Customer,"
		#define STR0003 "totalizing by product and selecting the hard currency for the Values."
		#define STR0004 "Z.Form "
		#define STR0005 "Management   "
		#define STR0006 "SALES STATISTICS      (Customer X Product)"
		#define STR0007 "CUSTOMER  CORPORATE NAME"
		#define STR0008 "PRODUCT         DESCRIPTION                INVOICE            ISSUE     UN   QUANTITY      UNIT PRICE             TOTAL    SALESMAN "
		#define STR0009 "CUSTOM.  CORPORATE NAME"
		#define STR0010 "Values in  "
		#define STR0011 "Selecting Records...     "
		#define STR0012 "CANCELLED BY THE OPERATOR  "
		#define STR0013 "Note: "
		#define STR0014 "TOTAL OF PRODUCT - "
		#define STR0015 "TOTAL OF CUSTOM. - "
		#define STR0016 "G R A N D   T O T A L                     ---->"
		#define STR0017 "RET"
		#define STR0018 "Sales statistics (Customer / Product)"
		#define STR0019 "This program lists the purchases made by the customer,"
		#define STR0020 "totaling by product and selecting the hard currency for the Amounts."
		#define STR0021 "Amounts on "
		#define STR0022 "PRODUCT TOTAL    - "
		#define STR0023 "CUSTOMER TOTAL   - "
		#define STR0024 "Sales R."
		#define STR0025 "RET"
		#define STR0026 "Selecting records ...    "	
		#define STR0027 "Customers"
		#define STR0028 "Outflow invoice items           "	
		#define STR0029 "Returns   "	
	#else
		Static STR0001 := "Estatisticas de Vendas (Cliente x Produto)"
		Static STR0002 := "Este programa ira emitir a relacao das compras efetuadas pelo Cliente,"
		Static STR0003 := "totalizando por produto e escolhendo a moeda forte para os Valores."
		Static STR0004 := "Zebrado"
		Static STR0005 := "Administracao"
		Static STR0006 := "ESTATISTICAS DE VENDAS (Cliente X Produto)"
		Static STR0007 := "CLIENTE   RAZAO SOCIAL"
		Static STR0008 := "PRODUTO         DESCRICAO                  NOTA FISCAL        EMISSAO   UN   QUANTIDADE    PRECO UNITARIO            TOTAL  VENDEDOR"
		Static STR0009 := "CLIENTE  RAZAO SOCIAL"
		#define STR0010  "Valores em "
		Static STR0011 := "Selecionando Registros..."
		Static STR0012 := "CANCELADO PELO OPERADOR"
		Static STR0013 := "Obs.: "
		Static STR0014 := "TOTAL DO PRODUTO - "
		Static STR0015 := "TOTAL DO CLIENTE - "
		Static STR0016 := "T O T A L   G E R A L                        ---->"
		Static STR0017 := "DEV"
		Static STR0018 := "Estatisticas de Vendas (Cliente x Produto)"
		Static STR0019 := "Este programa ira emitir a relacao das compras efetuadas pelo Cliente,"
		Static STR0020 := "totalizando por produto e escolhendo a moeda forte para os Valores."
		#define STR0021  "Valores em "
		Static STR0022 := "TOTAL DO PRODUTO - "
		Static STR0023 := "TOTAL DO CLIENTE - "
		#define STR0024  "Vendedor"
		Static STR0025 := "DEV"
		Static STR0026 := "Selecionando Registros..."	
		#define STR0027  "Clientes"	
		Static STR0028 := "Itens das Notas Fiscais de Saida"	
		Static STR0029 := "Devolucoes"	
	#endif
#endif

#ifndef SPANISH
#ifndef ENGLISH
	STATIC uInit := __InitFun()

	Static Function __InitFun()
	uInit := Nil
	If Type('cPaisLoc') == 'C'

		If cPaisLoc == "ANG"
			STR0001 := "Estatisticas de Vendas (Cliente x Artigo)"
			STR0002 := "Este Programa Irá Emitir A Relação Das Compras Efectuadas Pelo Cliente,"
			STR0003 := "A totalizar por artigo e a escolher a moeda forte para os valores."
			STR0004 := "Código de barras"
			STR0005 := "Administração"
			STR0006 := "Estatisticas De Vendas (cliente X Artigo)"
			STR0007 := "Cliente   Razao Social"
			STR0008 := "ARTIGO         DESCRICAO                  FACTURA        EMISSAO   UN   QUANTIDADE    PRECO UNITARIO            TOTAL  VENDEDOR"
			STR0009 := "Cliente  Razao Social"
			STR0011 := "A Seleccionar Registos..."
			STR0012 := "Cancelado Pelo Operador"
			STR0013 := "Obs.:"
			STR0014 := "TOTAL DO ARTIGO - "
			STR0015 := "Total do cliente - "
			STR0016 := "T o t a l   crial                        ---->"
			STR0017 := "Dev"
			STR0018 := "Estatisticas de Vendas (Cliente x Artigo)"
			STR0019 := "Este Programa Irá Emitir A Relação Das Compras Efectuadas Pelo Cliente,"
			STR0020 := "A totalizar por artigo e a escolher a moeda forte para os valores."
			STR0022 := "TOTAL DO ARTIGO - "
			STR0023 := "Total do cliente - "
			STR0025 := "Dev"
			STR0026 := "Seleccionando Registos..."	
			STR0028 := "Elementos das Facturas de Saída"	
			STR0029 := "Devoluções"	
		ElseIf cPaisLoc == "PTG"
			STR0001 := "Estatisticas de Vendas (Cliente x Artigo)"
			STR0002 := "Este Programa Irá Emitir A Relação Das Compras Efectuadas Pelo Cliente,"
			STR0003 := "A totalizar por artigo e a escolher a moeda forte para os valores."
			STR0004 := "Código de barras"
			STR0005 := "Administração"
			STR0006 := "Estatisticas De Vendas (cliente X Artigo)"
			STR0007 := "Cliente   Razao Social"
			STR0008 := "ARTIGO         DESCRICAO                  FACTURA        EMISSAO   UN   QUANTIDADE    PRECO UNITARIO            TOTAL  VENDEDOR"
			STR0009 := "Cliente  Razao Social"
			STR0011 := "A Seleccionar Registos..."
			STR0012 := "Cancelado Pelo Operador"
			STR0013 := "Obs.:"
			STR0014 := "TOTAL DO ARTIGO - "
			STR0015 := "Total do cliente - "
			STR0016 := "T o t a l   crial                        ---->"
			STR0017 := "Dev"
			STR0018 := "Estatisticas de Vendas (Cliente x Artigo)"
			STR0019 := "Este Programa Irá Emitir A Relação Das Compras Efectuadas Pelo Cliente,"
			STR0020 := "A totalizar por artigo e a escolher a moeda forte para os valores."
			STR0022 := "TOTAL DO ARTIGO - "
			STR0023 := "Total do cliente - "
			STR0025 := "Dev"
			STR0026 := "Seleccionando Registos..."	
			STR0028 := "Elementos das Facturas de Saída"	
			STR0029 := "Devoluções"	
		EndIf
		EndIf
	Return Nil
#ENDIF
#ENDIF
